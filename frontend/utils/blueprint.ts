...
import { IDecodedBlueprintData, IDecodedBlueprintBookData, IBlueprint, TBookItem, IBookItemBlueprint } from "../types"
...
export function decodeBlueprint(blueprintString: string): IDecodedBlueprintData | IDecodedBlueprintBookData {
  const versionlessString = blueprintString.substr(1)
  const compressed = Buffer.from(versionlessString, "base64")
  const decodedObject = JSON.parse(utf8Decoder.decode(pako.inflate(compressed))) as
